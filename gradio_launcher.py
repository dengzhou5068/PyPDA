...
```
    ...
    def process_pdb(self, protein_name: str, smiles: str = None):
        """处理PDB文件"""
        if not protein_name or not str(protein_name).strip():
        ...
        try:
            ...
            # 执行命令
            cmd = [
                sys.executable, "pypda.py", "pdb", "fetch",
                protein_name.strip(),
                "--output_dir", output_dir
            ]
...
def create_interface():
    ...
    with gr.Blocks(title="PyPDA - 蛋白质数据分析工具", theme=gr.themes.Soft()) as app:
            ...
            with gr.Row():
                with gr.Column():
                    ...
                    smiles_input = gr.Textbox(
                        label="SMILES字符串 (可选)",
                        placeholder="输入小分子的SMILES字符串，用于计算结构相似性并排序PDB结构\n例如: COc1ccccc1C(=O)Nc2ccc(OC(=O)C)cc2C(=O)O"
                    )
        ...
        process_pdb_btn.click(
            ...
            inputs=[pdb_protein_input, smiles_input],

...
class Logger:
    """日志管理类，负责统一错误处理和日志输出"""
    @staticmethod
    def log_error(error_msg: str, error_file_path: str = "error.txt") -> None:
        """记录错误信息到日志文件并打印

        Args:
            error_msg: 错误信息
            error_file_path: 错误日志文件路径，默认为 "error.txt"
        """
    ...
    @staticmethod
    def log_info(info_msg: str, info_file_path: str = "info.txt") -> None:
        """记录信息日志到日志文件并打印

        Args:
            info_msg: 信息内容
            info_file_path: 信息日志文件路径，默认为 "info.txt"
        """
